                        ...
                        switch( VLC_FOURCC(p_t35[3],p_t35[4],p_t35[5],p_t35[6]) )
                        {
                            case VLC_FOURCC('G', 'A', '9', '4'):
                                ...
                                else if( p_t35[7] == 0x06 && i_t35 >= 10 )
                                {
                                    ...
                                    const uint8_t *p_bar = &p_t35[9];
                                    if( vlc_popcount(i_flags) * 2 == i_t35 - 10 )
                                    {
                                        sei_data.itu_t35.type = HXXX_ITU_T35_TYPE_BAR;
                                        sei_data.itu_t35.u.bar.top = read_ga94_bar( i_flags & 0x08, &p_bar );
                                        sei_data.itu_t35.u.bar.bottom = read_ga94_bar( i_flags & 0x04, &p_bar );
                                        sei_data.itu_t35.u.bar.left = read_ga94_bar( i_flags & 0x02, &p_bar );
                                        sei_data.itu_t35.u.bar.right = read_ga94_bar( i_flags & 0x01, &p_bar );
                                        b_continue = pf_callback( &sei_data, cbdata );
                                    }
                                }
                                break;
