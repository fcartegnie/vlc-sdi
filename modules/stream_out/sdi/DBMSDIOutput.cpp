...
int DBMSDIOutput::doProcess()
{
    vlc_mutex_locker locker(&globallock);
    const vlc_tick_t preroll = CLOCK_FREQ*3/4;
...
int DBMSDIOutput::ProcessAudio(block_t *p_block)
{
    ...
    HRESULT result = p_output->ScheduleAudioSamples(
                ...
                p_block->i_pts, CLOCK_FREQ, &written);
...
int DBMSDIOutput::doProcessVideo(picture_t *picture, block_t *p_cc)
{
    ...
    result = p_output->ScheduleVideoFrame(pDLVideoFrame, picture->date, length, CLOCK_FREQ);
    if (result != S_OK) {
