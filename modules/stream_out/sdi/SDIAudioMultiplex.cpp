...
bool SDIAudioMultiplexConfig::addAssociation(int i_stream_id, std::vector<uint8_t> subframeslots)
{
    for(size_t i=0; i<mappings.size(); i++)
        if(mappings[i]->i_stream_id == i_stream_id)
            return false;
...
SDIAudioMultiplexBuffer *
    SDIAudioMultiplexConfig::getBufferForStream(int i_stream_id)
{
    for(size_t i=0; i<mappings.size(); i++)
    {
        if(mappings[i]->i_stream_id == i_stream_id)
