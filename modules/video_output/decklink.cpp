...
{
    uint8_t cc_count = p_vanc->i_data / 3;
    if (cc_count == 0)
        return;
    ...
    if (cc_count < 20) {
        ...
        for(int i = cc_count; i < 20; i++) {
            p_vanc->p_data[3*i + 0] = 0xfa;
            p_vanc->p_data[3*i + 0] = 0x00;
            p_vanc->p_data[3*i + 0] = 0x00;
        }
        cc_count = 20;
        p_vanc->i_data = 3 * 20;
    ...
    for (size_t i = 0; i < cc_count; i++) { // copy cc_data
        cdp[6+9+3*i+0] = p_vanc->p_data[3*i+0] /*| 0xfc*/; // marker bits + cc_valid
        cdp[6+9+3*i+1] = p_vanc->p_data[3*i+1];
        cdp[6+9+3*i+2] = p_vanc->p_data[3*i+2];
    }
...
#define ANCILLARY_CLOSED_CAPTIONS -1
